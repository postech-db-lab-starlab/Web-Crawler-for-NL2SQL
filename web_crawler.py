    ...
    def read_file(self, fname):
        with open(fname) as f:
            ...
            for line in lines:
                ...
                if len(attrs) != 2:
                    return False
                self.confs[attrs[0].strip()] = \
                        attrs[1].strip().split('#')[0].strip()
            return True
        return False
...
class db_manager:
    ...
    def connect_postgres(self, host, dbname, user, port, passwd):
        self.host, self.dbname, self.user, self.port = str(
            host), str(dbname), str(user), str(port)
        self.passwd = str(passwd)
        conn_string = "host='%s' dbname='%s' user='%s' port='%s'" % (
            self.host, self.dbname, self.user, self.port)
    ...
    def update_url_status(self, uid, status):
        stmt = "update url_info set status = %s where id = %s" % (
            str(status), str(uid))
        self.curs.execute(stmt)
...
class url_queue:
    ...
    def __init__(self, confs):
        ...
        self.min_date = date(
            int(date_info[0]), int(date_info[1]), int(date_info[2]))
    ...
    def make_google_query(self, keyword, date_range):
        url = 'https://www.google.co.kr/search?q=%s&sa=N&filter=0&num=100' % (
            keyword.strip())
    ...
    def inverse_date_range(self):
        ...
        for n in range(
                int((self.max_date - self.min_date).days / self.interval_date)):
    ...
    def insert_url(self, dm, url):
        ...
        uid = dm.insert(stmt)
        stmt = "insert into url_content(url_id, content) values(%s,'%s');" % (
            uid, url)
        dm.execute(stmt)
    ...
    def execute(self, wm, dm, keywords):
        ...
        for keyword_subset in keyword_subsets:
            ...
            for date_range in self.inverse_date_range():
                page = 1 
                google_query = self.make_google_query(keyword, date_range)
                while google_query is not None:
                    ...
                    page = page + 1
                    google_query = self.extract_google_query(html, page)
                    interval = random.randrange(
                        self.min_interval_time, self.max_interval_time)
...
class web_downloader():
    ...
    def insert_html(self, dm, urlinfo, html):
        stmt = "insert into html_content(url_id, content) values(%s, '%s')" % (
            urlinfo[0], html.replace("'", "''"))
        dm.execute(stmt)
...
class parser:
    ...
    def insert_candidate(self, dm, uid, position, content, status):
        stmt = "insert into candidate_info(url_id, position, status) values (%s, %s, %s) returning id;" % (
            str(uid), str(position), str(status))
        cid = dm.insert(stmt)
        content = content.replace("'", "''")
        stmt = "insert into candidate_content(candidate_id, content) values(%s,'%s');" % (
            str(cid), content.replace("'", "''"))
        dm.execute(stmt)
        ...
        for urlinfo in urlinfos:
            ...
            for (candidate, status) in self.content_filtering(candidates):
                self.insert_candidate(
                    dm, urlinfo[0], position, candidate, status)
                position += 1
            if position == 0:
...
class sql_extractor:
    ...
    def extract_start(self, text):
        ...
        for keyword in self.keywords:
            ...
            if minIdx == -1 or (idx >= 0 and minIdx > idx):
                minIdx = idx
        return minIdx
    ...
    def insert_sql(self, dm, uid, position, content):
        stmt = "insert into sql_info(url_id, position) values(%s, %s) returning id;" % (
            str(uid), str(position))
        sid = dm.insert(stmt)
        stmt = "insert into sql_content(sql_id, content) values(%s, '%s');" % (
            str(sid), content.replace("'", "''"))
        dm.execute(stmt)
...
class nl_extractor:
    ...
    def insert_nl(self, dm, uid, position, content):
        stmt = "insert into nl_info(url_id, position) values(%s, %s) returning id;" % (
            str(uid), str(position))
        sid = dm.insert(stmt)
        stmt = "insert into nl_content(nl_id, content) values(%s, '%s');" % (
            str(sid), content.replace("'", "''"))
        dm.execute(stmt)
        pass
